...
const Sidebar: React.FC = () => {
  ...
  return (
    <aside
      className={`min-h-screen bg-white border-r border-gray-200 flex flex-col justify-between py-8 shadow-md overflow-y-auto transition-all duration-300 
        ${collapsed ? "w-16" : "w-80"}`}
    >
      <div>
        <div className={`px-6 transition-all duration-300 ${collapsed ? "px-2" : "px-6"}`}>
              ...
              <nav className="flex flex-col gap-3">
                ...
                {navLinks.map(link => (
                  <Link href={link.href} key={link.href} legacyBehavior>
                    <a
                      className={`flex items-center w-full text-left px-5 py-3 rounded-xl font-semibold transition transform duration-150 text-lg ${active(link.href)
