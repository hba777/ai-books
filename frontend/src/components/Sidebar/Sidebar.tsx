...
const otherLinks = [
  { label: "Settings", href: "/settings", icon: <IoSettingsOutline className="text-base" /> },
  { label: "Help & Support", href: "/help", icon: <IoIosHelpCircleOutline className="text-base" /> },
];
...
const Sidebar: React.FC = () => {
  ...
  if (collapsed) {
    return (
      <aside className="min-h-screen w-16 bg-white border-r flex flex-col justify-between py-8 px-2 shadow-md overflow-y-auto transition-all duration-300">
        <div className="flex flex-col items-center gap-8 w-full">
          <button
            className="p-2 rounded-lg hover:bg-gray-100 transition self-center"
  ...
  return (
    <aside className="min-h-screen w-80 bg-white border-r border-gray-200 flex flex-col justify-between py-8 shadow-md overflow-y-auto transition-all duration-300">
      <div>
        <div className="px-6">
          ...
          <nav className="flex flex-col gap-3">
            ...
            {navLinks.map(link => (
              <Link href={link.href} key={link.href} legacyBehavior>
                <a
                  className={`relative flex items-center w-full text-left px-5 py-3 rounded-xl font-semibold transition transform duration-150 text-lg ${active(link.href)
                ...
                >
                  ...
                  <span className={`flex items-center justify-center w-10 h-10 rounded-lg mr-4 ${active(link.href) ? "bg-[#2563eb] text-white" : "bg-gray-100"}`}>
                    {link.icon}
                  </span>
                  <span className="flex flex-col items-start">
                    <span>{link.label}</span>
                    <span className="text-xs font-normal text-gray-400 leading-tight mt-0.5">{link.subtitle}</span>
                  </span>
                </a>
              </Link>
            ))}
            ...
            {otherLinks.map(link => (
              <Link href={link.href} key={link.href} legacyBehavior>
                <a
                  className={`relative flex items-center w-full text-left px-4 py-2 rounded-lg font-medium transition transform duration-150 ${active(link.href)
                    ? "bg-[#eff6ff] text-gray-700 shadow-md scale-[1.04]"
                    : "text-gray-700 hover:bg-gray-100 hover:shadow hover:scale-[1.03]"}`}
                >
                  {active(link.href) && (
                  ...
                  <span className={`flex items-center justify-center w-8 h-8 rounded-lg mr-3 ${active(link.href) ? "bg-[#2563eb] text-white" : "bg-gray-100"}`}>
                    {link.icon}
                  </span>
                  {link.label}
                </a>
